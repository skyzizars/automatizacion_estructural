...
def acero_maximo_pb(rho_b, As_max):
    text = r'''
        $ \rho_{b} = \frac{0.85 * fc * \beta_{1}} {fy} * \frac{6000}{6000+fy} = var:rho_b \\
        \rho_{max} = 0.75*\rho_{b} = var:rho_max \\
        As_{max} = \rho_{b}*b*d = var:As_max cm^{2}
    '''  
    rho_max = round(0.75 * rho_b,4)
